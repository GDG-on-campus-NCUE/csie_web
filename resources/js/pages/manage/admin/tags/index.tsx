...
export default function ManageAdminTagsIndex() {
    ...
    const toggleActivation = async (tag: ManageTag) => {
        ...
        try {
            ...
            showSuccess(tag.is_active ? '已停用標籤' : '已啟用標籤');
            ...
            <form className="flex flex-wrap items-center gap-2" onSubmit={(event: FormEvent<HTMLFormElement>) => event.preventDefault()}>
                <div className="flex items-center gap-2">
                    <Input
                        ...
                        placeholder="搜尋標籤名稱或代碼"
                        className="w-56"
                        aria-label="搜尋標籤"
                    />
                    <Button type="button" size="sm" className="gap-1" onClick={() => applyFilters({ keyword: filterForm.keyword })}>
                        <Filter className="h-4 w-4" />
                        套用
                    </Button>
                </div>
                <Select value={filterForm.context} onChange={handleContextChange} className="w-40" aria-label="模組篩選">
                ...
                <Select value={filterForm.status} onChange={handleStatusChange} className="w-36" aria-label="狀態篩選">
                ...
                <Select value={filterForm.per_page} onChange={handlePerPageChange} className="w-28" aria-label="每頁筆數">
    ...
    return (
        <>
            ...
            <ManagePage
                title={pageTitle}
                description="集中管理公告、附件與空間所使用的標籤，支援合併與拆分等維運操作。"
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="border-neutral-200/70">
                                    ...
                                    <TableHead className="min-w-[220px] text-neutral-600">標籤名稱</TableHead>
                                    <TableHead className="min-w-[160px] text-neutral-600">英文名稱</TableHead>
                                    <TableHead className="min-w-[140px] text-neutral-600">模組</TableHead>
                                    <TableHead className="min-w-[120px] text-right text-neutral-600">使用次數</TableHead>
                                    <TableHead className="min-w-[160px] text-neutral-600">最後使用</TableHead>
                                    <TableHead className="min-w-[100px] text-neutral-600">狀態</TableHead>
                                    <TableHead className="w-14" aria-label="操作" />
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {hasTags ? (
                                    tags.data.map((tag) => {
                                        ...
                                        return (
                                            <TableRow key={tag.id} className="border-neutral-200/60 hover:bg-neutral-50/70">
                                                ...
                                                <TableCell className="text-sm text-neutral-600">{tag.name_en || '—'}</TableCell>
                                                <TableCell className="text-sm text-neutral-600">{tag.context_label}</TableCell>
                                                <TableCell className="text-right font-medium text-neutral-700">
                                                    {colorFormatter.format(tag.usage_count ?? 0)}
                                                </TableCell>
                                                <TableCell className="text-sm text-neutral-600">{formatDateTime(tag.last_used_at, locale)}</TableCell>
                                                <TableCell>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            {abilities.canUpdate ? (
                                                                <DropdownMenuItem onSelect={() => openEditDialog(tag)}>
                                                                    <Edit3 className="mr-2 h-4 w-4" /> 編輯
                                                                </DropdownMenuItem>
                                                            ) : null}
                                                            <DropdownMenuItem onSelect={() => setSplitTag(tag)}>
                                                                <SplitSquareVertical className="mr-2 h-4 w-4" /> 拆分
                                                            </DropdownMenuItem>
                                                            {abilities.canUpdate ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={8} className="py-10">
                                            <TableEmpty
                                                title="目前沒有符合條件的標籤"
                                                description="您可以調整篩選條件，或建立新的標籤來分類後台資料。"
                                                action={abilities.canCreate ? (
                                                    <Button size="sm" className="gap-1" onClick={() => setCreateOpen(true)}>
                                                        <Plus className="h-4 w-4" /> 新增標籤
                                                    </Button>
                                                ) : undefined}
                                            />
                                        </TableCell>
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-xl">
                <DialogHeader>
                    <DialogTitle>{mode === 'create' ? '新增標籤' : '編輯標籤'}</DialogTitle>
                    <DialogDescription>
                        {mode === 'create'
                            ? '設定標籤的中英文名稱與顏色，建立後可用於公告、附件與空間等模組。'
                            : '更新標籤資訊並調整狀態，變更後會即時套用至所有關聯資料。'}
                    </DialogDescription>
                </DialogHeader>

                <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                    <FormField label="套用模組" required error={errors.context} direction="horizontal">
                        <Select
                    ...
                    <div className="flex items-center justify-end gap-2">
                        ...
                        <Button type="submit" disabled={isSubmitting || (!values.context && mode === 'create')}>
                            {isSubmitting ? '處理中…' : mode === 'create' ? '建立' : '儲存變更'}
                        </Button>
                    </div>
                </form>
            </DialogContent>
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        選取要保留的標籤，其餘標籤的關聯資料將轉移至保留標籤，並自動停用。
                    </DialogDescription>
                </DialogHeader>

                <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                    <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-3 text-sm text-neutral-600">
                        <span className="font-medium text-neutral-800">已選擇 {selectedTags.length} 個標籤</span>
                    ...
                    <FormField label="保留標籤" required direction="horizontal">
...
function SplitDialog({ open, onOpenChange, tag, onSubmit }: SplitDialogProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-lg">
                <DialogHeader>
                    <DialogTitle>拆分標籤</DialogTitle>
                    <DialogDescription>
                        以逗號或換行輸入多個新標籤名稱，可快速建立細分類別。
                    </DialogDescription>
                </DialogHeader>

                <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                    <div className="rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-600">
                        <span className="font-medium text-neutral-800">原標籤</span>
                    ...
                    <FormField label="新標籤名稱" required direction="horizontal" description="使用逗號或換行分隔，例如：國際交流,就業資訊,活動花絮">
                        <textarea
                            value={names}
