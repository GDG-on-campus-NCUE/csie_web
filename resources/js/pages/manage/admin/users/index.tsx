...
export default function ManageAdminUsersIndex() {
    ...
    const applyFilters = useCallback((overrides: Partial<FilterFormState> = {}, options: { replace?: boolean } = {}) => {
        const payload: Record<string, string | number | string[] | null> = {
            ...
            statuses: (overrides.status ?? filterForm.status) ? [overrides.status ?? filterForm.status] : null,
    ...
    }, [filterForm.keyword, filterForm.roles, filterForm.status, filterForm.space, filterForm.sort, filterForm.direction, filterForm.per_page]);
    ...
    const handleRoleFilterChange = (value: string, checked: boolean) => {
        setFilterForm(prev => {
    ...
    const handleClearFilters = () => {
        ...
        applyFilters(resetState, { replace: true });
    };
    ...
    return (
        <>
            ...
            <ManagePage
                title={pageTitle}
                description={t('users.description', '集中管理系統帳號的角色、權限與狀態。')}
                breadcrumbs={breadcrumbs}
            >
                <FilterPanel
                    title={t('users.filters.title', '篩選條件')}
                    collapsible={true}
                    defaultOpen={true}
                    onApply={() => applyFilters()}
                    onReset={handleClearFilters}
                    applyLabel={t('users.filters.apply', '套用篩選')}
                    resetLabel={t('users.filters.reset', '重設')}
                >
                    <div className="grid grid-cols-12 gap-3">
                        {/* 搜尋框 */}
                        <div className="col-span-12 md:col-span-4 space-y-2">
                            <label className="text-sm font-medium text-neutral-700">
                                {t('users.filters.keyword_label', '搜尋使用者')}
                            </label>
                            <Input
                                    ...
                                    >
                                        <span className="truncate">
                                            {filterForm.roles.length > 0
                                                ? t('users.filters.roles_selected', '已選 :count 個角色', {
                                                      count: filterForm.roles.length,
                                                  })
                                                : t('users.filters.roles_all', '全部角色')}
                                        </span>
                        ...
                        <div className="col-span-12 md:col-span-4 space-y-2">
                            <label className="text-sm font-medium text-neutral-700">
                                {t('users.filters.space_label', '所屬空間')}
                            </label>
                            <Select
                        ...
                        <div className="col-span-12 md:col-span-4 space-y-2">
                            <label className="text-sm font-medium text-neutral-700">
                                {t('users.filters.sort_label', '排序方式')}
                            </label>
                            <Select
                        ...
                        <div className="col-span-12 md:col-span-4 space-y-2">
                            <label className="text-sm font-medium text-neutral-700">
                                {t('users.filters.direction_label', '排序方向')}
                            </label>
                            <Select
                        ...
                        <div className="col-span-12 md:col-span-4 space-y-2">
                            <label className="text-sm font-medium text-neutral-700">
                                {t('users.filters.per_page_label', '每頁筆數')}
                            </label>
                            <Select
                ...
                <ManageToolbar
                    ...
                    secondary={
                        <div className="flex flex-col gap-2 md:flex-row md:items-center">
                            {/* 批次操作 */}
                            {abilities.canUpdate || abilities.canAssignRoles ? (
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button
                                            variant="outline"
