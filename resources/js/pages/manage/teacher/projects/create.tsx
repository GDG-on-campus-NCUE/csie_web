...
import type { ManageProjectFormData } from '@/types/manage/teacher';
...
interface ManageTeacherProjectsCreatePageProps {
...
export default function ManageTeacherProjectsCreate() {
    const page = usePage<ManageTeacherProjectsCreatePageProps>();
    const { spaces, tagOptions } = page.props;
    const { t } = useTranslator('manage');

    const { data, setData, post, processing, errors } = useForm<ManageProjectFormData>({
        title: '',
        title_en: '',
        sponsor: '',
        principal_investigator: page.props.auth.user.name,
        start_date: '',
        end_date: '',
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: t('teacher.projects.create', '新增計畫'),
            href: '/manage/teacher/projects/create',
        },
    ];

    const pageTitle = t('teacher.projects.create', '新增研究計畫');

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post('/manage/teacher/projects');
    };
    ...
    return (
        <>
            ...
            <ManagePage
                title={pageTitle}
                description={t('teacher.projects.create_description', '填寫研究計畫的基本資訊。')}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex items-center justify-end gap-4">
                        <Button type="button" variant="outline" asChild>
                            <Link href="/manage/teacher/projects">{t('layout.cancel', '取消')}</Link>
                        </Button>
...
ManageTeacherProjectsCreate.layout = (page: ReactElement) => <AppLayout>{page}</AppLayout>;
