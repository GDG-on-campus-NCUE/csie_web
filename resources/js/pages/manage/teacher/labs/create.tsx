...
import type { ManageLabFormData } from '@/types/manage/teacher';
...
type ManageTeacherLabsCreatePageProps = SharedData & {
    users: User[];
};

export default function ManageTeacherLabsCreate() {
    const page = usePage<ManageTeacherLabsCreatePageProps>();
    ...
    const { data, setData, post, processing, errors } = useForm<ManageLabFormData & { members?: number[] }>({
        name: '',
        name_en: '',
        field: '',
        location: '',
    ...
    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        post('/manage/teacher/labs');
    };

    const handleCancel = () => {
        if (confirm(t('common.confirm_cancel', '確定要取消嗎？未儲存的變更將遺失。'))) {
            router.visit('/manage/teacher/labs');
        }
    };
    ...
    const pageTitle = t('teacher.labs.create', '新增實驗室');
    ...
    return (
        <>
            ...
            <ManagePage
                title={pageTitle}
                description={t('teacher.labs.create_description', '填寫實驗室基本資訊並選擇成員。')}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onChange={(e: ChangeEvent<HTMLSelectElement>) => {
                                            ...
                                            setData('principal_investigator_id', value ? Number(value) : undefined);
                                        }}
                                    >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e: ChangeEvent<HTMLInputElement>) => {
                                            const value = e.target.value;
                                            setData('capacity', value ? Number(value) : undefined);
                                        }}
                                        placeholder="30"
                                    />
...
ManageTeacherLabsCreate.layout = (page: ReactElement) => <AppLayout>{page}</AppLayout>;
